...
if not hasattr(core, 'bm3dcuda_rtc'):
    raise RuntimeError("PMOad Error: bm3dcuda_rtc plugin not found.")
...
if not hasattr(core, 'nnedi3cl'):
    raise RuntimeError("PMOyster Error: nnedi3cl plugin not found.")
...
# === SCENE CHANGE CHUNKING ===
def SCDetect(clip, threshold=0.140):
    return core.misc.SCDetect(clip, threshold=threshold)

def ProcessInScenes(src, func, **kwargs):
    sc = SCDetect(src)
    chunks = []
    start = 0
    for n in range(1, src.num_frames):
        if sc.get_frame(n).props.get("_SceneChange", 0):
            chunk = src[start:n]
            filtered = func(chunk, **kwargs)
            chunks.append(filtered)
            start = n
    chunk = src[start:]
    filtered = func(chunk, **kwargs)
    chunks.append(filtered)
    return Splice(chunks, mismatch=False)
...
def _freq_merge(low, hi, sbsize, slocation):
    ...
    return _ensure_float32(MergeDiff(low_filtered, hif))
...
def _nl_means(src, d, a, s, h, rclip, level):
    a_adaptive = 8 if level == 0 else 6
...
def _gen_block_mask(src):
    ...
    return core.std.CropAbs(mask_tiled, luma.width, luma.height, 0, 0)

# === INTERNAL SUPER-CLIP GENERATOR  ===
def _make_super(clip, pel=4):
    src_pad = AddBorders(clip, 128, 128, 128, 128)
    
    doubled = Transpose(NNEDI(Transpose(NNEDI(src_pad, **nnedi_args)), **nnedi_args))
    if pel == 4:
        doubled = Transpose(NNEDI(Transpose(NNEDI(doubled, **nnedi_args)), **nnedi_args))
    
    return doubled

# === BASIC TEMPORAL DENOISE ===
def _basic(src, radius, pel, sad, short_time, level):
    src_pad = AddBorders(src, 128, 128, 128, 128)
    super_clip = _make_super(src, pel=pel)
...
def _thr_merge(flt, src, ref=None, thr=0.03125, elast=None, fast=False):
    ...
    return Expr([flt, ref, UDN, src], f"x y - abs {thr} {elast} + < z a ?")
...
def _bm3d_fast(clip, ref, sigma, block_step, bm_range, ps_num, ps_range, radius):
    return BM3Dv2(clip, ref=ref, sigma=sigma,
                  ...
                  bm_error_s="SAD", transform_2d_s="Haar")
...
# === PUBLIC API ===
def Basic(src, *, radius=None, pel=4, sad=None, short_time=False, level=0,
    ...
    return _basic(src, radius, pel, sad, short_time, level)
...
def Process(src,
         ...
         use_sc=True):      
    """
    All-in-one Oyster with optional scene-change chunking.

    use_sc=False  → Instant first frame (great for preview/comparison)
    use_sc=True   → Full scene-aware processing (best quality, slight delay)
    pel=2         → Good speed/quality balance (default)
    pel=4         → ~3× slower, better motion estimation for complex motion
    """
    def process_chunk(chunk):
        den = Basic(chunk,
                    radius=None, pel=pel, sad=None,
                    short_time=short_time, level=level,
                    format=format, correction=correction)

        if process == "deblock":
            return Deblocking(den, den, level=level, format=format, correction=correction)
        elif process == "dering":
            return Deringing(den, den, level=level, format=format, correction=correction)
        elif process == "destaircase":
            return Destaircase(den, den, level=level, format=format, correction=correction)
        else:  # combined
            der = Deringing(den, den, level=level, format=format, correction=correction)
            des = Destaircase(der, der, level=level, format=format, correction=correction)
            return des

    if not use_sc:
        return process_chunk(src)

    old_sc = SCDetect
    def SCDetect_custom(clip):
        return old_sc(clip, threshold=sc_threshold)
    globals()['SCDetect'] = SCDetect_custom

    return ProcessInScenes(src, process_chunk)
