...
def _nl_means(src, d, a, s, h, rclip, level):
    """NLMeans with temporal padding - following Oyster blueprint pattern"""
    
    # Adaptive search area for ultra-quality levels (improvement #4)
    a_adaptive = [6, a, a, a, a][level]  # Smaller search area at level 0
    s_adaptive = [3, s, s, s, s][level]  # Smaller similarity window at level 0
    ...
    pad = AddBorders(src, a_adaptive+s_adaptive, a_adaptive+s_adaptive, 
                     a_adaptive+s_adaptive, a_adaptive+s_adaptive)
    pad = duplicate(pad)
    
    if rclip:
        rclip = AddBorders(rclip, a_adaptive+s_adaptive, a_adaptive+s_adaptive,
                          a_adaptive+s_adaptive, a_adaptive+s_adaptive)
        rclip = duplicate(rclip)
    
    nlm = NLMeans(pad, d=d, a=a_adaptive, s=s_adaptive, h=h, channels="Y", wref=1.0, rclip=rclip)
    crop = CropRel(nlm, a_adaptive+s_adaptive, a_adaptive+s_adaptive, 
                   a_adaptive+s_adaptive, a_adaptive+s_adaptive)
...
def _deblocking(src, ref, radius, h, sigma, lowpass, level):
    """Deblocking using difference-based processing"""
    mask = _gen_block_mask(src)
    ...
    # Difference-based processing (following Oyster blueprint)
    dif = MakeDiff(ref, cleansed)
    dif = BM3D(dif, ref=cleansed, sigma=sigma_scaled,
               block_step=block_step, bm_range=bm_range,
               ps_num=ps_num, ps_range=ps_range, radius=radius)
    
    if radius > 0:
        dif = dif.bm3d.VAggregate(radius=radius)
    
    dif = _ensure_float32(dif)
    cleansed = MergeDiff(cleansed, dif)
    
    # Second pass for quality levels
    if level < 3:
        dif = MakeDiff(ref, cleansed)
        dif = BM3D(dif, ref=cleansed, sigma=sigma_scaled*0.75,
                   block_step=block_step, bm_range=bm_range,
                   ps_num=ps_num, ps_range=ps_range, radius=0)
        dif = _ensure_float32(dif)
        cleansed = MergeDiff(cleansed, dif)
    
    # Frequency merge both src and ref
